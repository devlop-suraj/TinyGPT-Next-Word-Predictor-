...
def load_corpus(file_name="corpus.txt"):
    """
    Loads training sentences from a text file.
    Each line in the file becomes one string in the list.
    """
    try:
...
block_size = 6 #it means whenever our model is predicting the next word the model will see how much previous word
embedding_dim = 32 #32 values for each word means dimension.
n_heads = 2  #self attention heads
n_layers = 2 
...
if start_word in word2idx:
    ...
    print(f"Error: '{start_word}' is not in the vocabulary.")
    exit()
